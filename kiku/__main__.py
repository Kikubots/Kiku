...
print("kiku has been deployed! ")

print("Setting up TGBot")
path = "kiku/plugins/mybot/*.py"
files = glob.glob(path)
for name in files:
    with open(name) as f:
        path1 = Path(f.name)
...
if LOAD_MYBOT == "True":
    path = "kiku/plugins/mybot/pmbot/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as f:
            path1 = Path(f.name)
...
print(
    """
                ----------------------------------------------------------------------
                    Lɪᴏɴ X ʜᴀs ʙᴇᴇɴ ᴅᴇᴘʟᴏʏᴇᴅ, ᴅᴏ ᴠɪsɪᴛ @kikusupport !!
                    ʟɪᴏɴ ᴠᴇʀ: V2.2
                    ©Tᴇᴀᴍ ʟɪᴏɴ
                ----------------------------------------------------------------------
"""
